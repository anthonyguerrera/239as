...
class DQN:
    ...
    def train(self, n_episodes:int = 1000,validate_every:int = 100, n_validation_episodes:int = 10, n_test_episodes:int = 10,save_every:int = 100):
        ...
        for episode in range(n_episodes):
            ...
            print(f"Episode: {episode}: Time: {time.time() - start_time} Total Reward: {total_reward} Avg_Loss: {loss}")
    ...
    def _optimize_model(self):
        ...
        # Combine Q values for current states
        q_values = self.model(states).gather(1, actions)        
        # Combine Q values for next states using the target model
        with torch.no_grad():
